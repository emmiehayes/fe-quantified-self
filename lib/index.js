...
const postFood = () => {
  ...
  fetch(`https://limitless-everglades-18138.herokuapp.com/api/v1/foods`, postPayload(newFoodName, newFoodCalories))
  .then(handleResponse)
  .then(getAllFoods)
  .then(clearInput)
  .catch(errorLog)
...
const postFoodsToMeal = (foodsChecked, mealId) => {
  ...
  for (var i = 0; i < length; i++) {
    fetch(`https://limitless-everglades-18138.herokuapp.com/api/v1/meals/${mealId}/foods/${foodsChecked[i]}`, {method: "POST"})
    .then(handleResponse)
    .catch(errorLog)
  }
}
