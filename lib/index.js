...
$("#interactive-list").on('click', '.recipe-btn', function (event) {
  let foodName = function () { return this.className }
  let foodsChecked = $(":checkbox:checked").map(foodName).get()
...
const fetchRecipes = (foodsChecked) => {
  recipeView()
  var YUMMLY_API_KEY = config.YUMMLY_API_KEY
  var YUMMLY_APP_ID = config.YUMMLY_APP_ID

  var formatChecked = foodsChecked.join("+")
  fetch(`http://api.yummly.com/v1/api/recipes?_app_id=${YUMMLY_APP_ID}&_app_key=${YUMMLY_API_KEY}&requirePictures=true&q=${formatChecked}&maxResult=5`)
    .then(handleResponse)
...
const populateRecipes = (recipes) => {
  return recipes.matches.forEach((recipe) => {
    $(".recipe-data").append(`
